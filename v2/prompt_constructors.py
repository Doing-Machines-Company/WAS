...
def construct_memory_prompt(intent: str, last_action: Action, old_obs: AxObservation, new_obs: AxObservation, model_name: str) -> str: # NOT NEEDED FOR MemGPT
    if model_name.startswith('gpt'):
        messages = [{"role": "system",
                    ...
                    {"role": "system",
                     "content": f"First look at the task, then you must generate subtasks that can help you complete this task, reasong through these step-by-step. If any of the information in only the new accessibility tree is relevant to the task, that is a INFORMATION MEMORY you want to reply with. Then look at the last action performed, if the two accessibility trees indicates that the action was successful. Only include brief summaries of successful actions that complete IMPORTANT SUBTASKS as a TASK MEMORY you want to reply with. "},
                    {"role": "system",
                     "content": "Give your final reply in the form '''info memory reply you want|task memory reply you want''', where | is used to delimit the information memory and task memory you want to reply with. If you don't want to store a field, just reply with that side empty. Remember to enclose your final reply with '''."}
                    ]
