...
async def parse_and_clean(elements, parent_node):
    ...
    for element in elements:
        ...
        if href and url_depth(aggressive_url_norm(href)) <= 1 and aggressive_url_norm(href).endswith(".html") and (
                aggressive_url_norm(href) != parent_node.url):
            continue
...
async def interact_element_by_xpath(page, xpath, interaction_info, html, node):
    ...
    child_node = IntrastateWebPageNode(url=normalize_url(page.url), edge=edge_info, private=difference,
                                       acc_tree=after_tree)
...
class IntrastateWebPageNode:
    ...
    def __init__(self, url=None, edge=None, private=None, acc_tree=None,
                 embedding=None):  # represented by url and action, action taken at url/state
        self.url = url
...
async def reduce_duplicate_elements(elements, parent_node):  # same url + same visible text = BAD!
    ...
    for element in elements:
        ...
        if composite_key_visible not in IntrastateWebPageNode.unique_interacts_visible or (
                composite_key_html not in IntrastateWebPageNode.unique_interacts_html and visible_text.strip() == ''):
