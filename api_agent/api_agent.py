...
from api_agent_classes import APIType, APIAction
...
class APIAgent(ABC):
    ...
    async def action_repeated(self, new_actions: list[APIAction]):
        if self.last_action is None:
            return False
        return set(self.last_action) == set(new_actions)
    ...
    async def run(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                chosen_actions = action_out_call.parsed_output

                if not chosen_actions or await self.action_repeated(chosen_actions):
                ...
                await self.handle_actions(chosen_actions)

                self.last_action = chosen_actions
                gc.collect()
